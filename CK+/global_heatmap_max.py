...
CLASSES_FOR_HEATMAP = [cls for cls in CLASS_NAMES if cls != 'ig']  # Exclude contempt
IMAGE_SIZE = 48
...
GLOBAL_OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "global_heatmap_max")
SAMPLE_OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "sample_masked_images_global_max")

# ---------------------- Load Per-Class Heatmaps ----------------------
heatmaps = []
for cls in CLASSES_FOR_HEATMAP:
    path = os.path.join(HEATMAP_DIR, f"{cls}_heatmap.npy")
    heatmap = np.load(path)  # shape: (48, 48)
    heatmaps.append(heatmap)
    print(f"  ✓ Loaded heatmap for '{cls}'")

print(f"\nExcluded 'contempt' from global max heatmap calculation")

# Compute global max heatmap
...
plt.title("Global Per-Pixel Importance Heatmap (Max)")
plt.colorbar()
plt.tight_layout()
plt.savefig(os.path.join(GLOBAL_OUTPUT_DIR, "global_max_heatmap.png"))
plt.close()

np.save(os.path.join(GLOBAL_OUTPUT_DIR, "global_max_heatmap.npy"), global_heatmap)
...
np.save(os.path.join(GLOBAL_OUTPUT_DIR, "global_max_binary_mask.npy"), binary_mask)

plt.imshow(binary_mask, cmap='gray')
plt.title(f"Global Binary Mask (Top {THRESHOLD_PERCENT}% pixels)")
plt.tight_layout()
plt.savefig(os.path.join(GLOBAL_OUTPUT_DIR, "global_max_binary_mask.png"))
plt.close()
...
print("\nEvaluating on globally masked test set (max)...")
...
print(f"\nGlobal masked test accuracy (max, top {THRESHOLD_PERCENT}% pixels): {acc:.2f}%")
...
for cls in CLASS_NAMES:
    ...
    plt.title(f"Class {cls} - Global Masked (Max)")
    plt.axis('off')
    save_path = os.path.join(SAMPLE_OUTPUT_DIR, f"class_{cls}_masked.png")
    plt.savefig(save_path, bbox_inches='tight')
    plt.close()

print(f"Saved random masked samples to '{SAMPLE_OUTPUT_DIR}/' directory.")
