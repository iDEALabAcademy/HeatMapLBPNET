#!/usr/bin/env python3
"""
Train LBPNet with original LBP layer on AMNIST
Adapted for AMNIST dataset and grayscale conversion
"""
...
PRESETS = {
    # CK+ dataset with cropped images (37x37)
    "paper_ckplus_rp_cropped": {
        # model selection (string -> model builder in lbpnet.models)
        "model": "lbpnet_rp",

        # data-splitting / dataset-level options
        "data": {
            "variant": "cropped",      # use cropped CK+ dataset
            "train_ratio": 0.80,       # fraction of data used for training
            "val_ratio": 0.10,         # fraction of data used for validation
            # test uses remaining 10%
            "seed": 42                 # split RNG seed for reproducibility
        },

        # expected input image size (CK+ cropped is 37x37)
        "image_size": 37,
        ...
        "lbp_layer": {
            ...
            "offset_init_std": 0.02       # stddev for offset initialization
        },

        # Block/stage configuration (how many stages, channels, where to downsample)
        "blocks": {
            "stages": 3,                      # number of stages
            "channels_per_stage": [39, 40, 80], # output channels per stage
            "downsample_at": [1, 2],          # stage indices after which to downsample (0-based)
        ...
        "head": {
            ...
            "num_classes": 7,     # CK+ has 7 classes (emotions)
            "use_bn": True        # whether to use BatchNorm in classifier head
        },

        # High-level training hyperparameters (adjusted for smaller CK+ dataset)
        "training": {
            "epochs": 500,             # total training epochs (more epochs for small dataset)
            "batch_size": 16,          # per-device batch size (smaller for small dataset)
            "lr": 5e-4,                # base learning rate (lower for small dataset)
            "weight_decay": 1e-4,      # base weight decay (lower to avoid over-regularization)
            "lr_scheduler": "cosine",  # lr schedule: "cosine" or "step"
            "warmup_epochs": 10,       # warmup duration in epochs (if scheduler supports it)
            "patience": 150,            # early stopping patience (if used)
            "min_delta": 5e-4,         # minimum improvement threshold for early stopping
            "label_smoothing": 0.05    # label smoothing factor (lower for small dataset)
        },
        ...
        "optim": {
            ...
            "lr": 5e-4,              # initial lr for optimizer (lowered for small CK+ dataset)
            "weight_decay": 1e-4,    # base weight decay for optimizer (lowered for CK+)
        ...
        "freeze_schedule": {
            "freeze_offsets_epochs": 0,    # freeze offset learning for first N epochs
            "freeze_gates_extra_epochs": 0,# extra epochs to keep gates frozen (guard)
            "freeze_gate_epochs": 0        # gate freeze period used during training schedule
        },
...
def get_config():
    ...
    if yaml_path and os.path.isfile(yaml_path):
        ...
        # Default preset changed to AMNIST cropped preset
...
def train_original_model():
    print("🚀 Starting training LBPNet with the original LBP layer on CK+ (cropped)...")
    ...
    # Create AMNIST datasets
    print("📊 Creating CK+ datasets...")
    ...
    ckpt_default = os.path.join('./outputs_CKplus_cropped', 'best_model.pth')
    ...
    # Output dir
    output_dir = "./outputs_CKplus_cropped"
